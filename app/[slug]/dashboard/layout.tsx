...
const TruckIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="1" y="3" width="15" height="13" rx="2"/><path d="M16 8h4l3 3v5h-7z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
const ExpenseIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 1v22"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7H14.5a3.5 3.5 0 0 1 0 7H6"/></svg>
...
const TicketIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 10V6a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v4"/><path d="M3 14v4a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-4"/><path d="M8 12h8"/><path d="M12 8v8"/></svg>
const DocIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><path d="M8 13h8"/><path d="M8 17h6"/></svg>
const RequestIcon = () => <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>

function toModuleList(value: any): string[] {
  if (!value) return []
...
export default function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  function refreshEnabledModules(nextSettings?: any) {
    const subRaw = localStorage.getItem('subscription')
    let subscription: any = null
    if (subRaw) {
      try { subscription = JSON.parse(subRaw) } catch {}
    }
    const fromSettings = toModuleList(nextSettings?.enabled_modules)
    const fromSub = toModuleList(subscription?.enabled_modules || subscription?.modules || subscription?.plan_modules)
    const merged = fromSettings.length > 0 ? fromSettings : fromSub
  ...
  const ALL_NAV = [
    { label: 'Tableau de bord', href: `/${slug}/dashboard`, section: 'Général', icon: DashIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'dashboard' },
    { label: 'Factures', href: `/${slug}/dashboard/factures`, section: 'Gestion', icon: BillIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'billing' },
    { label: 'Clients', href: `/${slug}/dashboard/clients`, section: 'Gestion', icon: ClientIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'clients' },
    { label: 'Paiements', href: `/${slug}/dashboard/paiements`, section: 'Gestion', icon: PayIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'payments' },
    { label: 'Produits', href: `/${slug}/dashboard/produits`, section: 'Gestion', icon: ProdIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'catalog' },
    { label: 'Stock', href: `/${slug}/dashboard/stock`, section: 'Gestion', icon: StockIcon, roles: ['superadmin','admin','employe','owner'], platformAdminOnly: false, moduleKey: 'stock' },
    { label: 'Fournisseurs', href: `/${slug}/dashboard/fournisseurs`, section: 'Gestion', icon: TruckIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'suppliers' },
    { label: 'Depenses', href: `/${slug}/dashboard/depenses`, section: 'Gestion', icon: ExpenseIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'expenses' },
    { label: 'Tickets', href: `/${slug}/dashboard/tickets`, section: 'Relation Client', icon: TicketIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'tickets' },
    { label: 'Demandes', href: `/${slug}/dashboard/demandes`, section: 'Relation Client', icon: RequestIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'service_requests' },
    { label: 'Documents', href: `/${slug}/dashboard/documents`, section: 'Relation Client', icon: DocIcon, roles: ['superadmin','admin','employe','lecteur','owner'], platformAdminOnly: false, moduleKey: 'documents' },
    { label: 'Utilisateurs', href: `/${slug}/dashboard/utilisateurs`, section: 'Administration', icon: UserIcon, roles: ['superadmin','admin','owner'], platformAdminOnly: false, moduleKey: 'users' },
    { label: 'Paramètres', href: `/${slug}/dashboard/parametres`, section: 'Administration', icon: SettingIcon, roles: ['superadmin','admin','owner'], platformAdminOnly: false, moduleKey: 'settings' },
    { label: 'Admin RS', href: `/${slug}/dashboard/admin-platform`, section: 'RS Comptabilité', icon: ShieldIcon, roles: ['owner'], platformAdminOnly: true, moduleKey: 'admin_platform' },
  ]
  const SECTIONS = ['Général', 'Gestion', 'Relation Client', 'Administration', 'RS Comptabilité']
