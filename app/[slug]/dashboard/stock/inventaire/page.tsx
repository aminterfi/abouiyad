...
export default function InventairePage() {
  ...
  async function applyInventory() {
    if (!confirm('Appliquer les ecarts d inventaire ? Les stocks seront ajustes automatiquement.')) return
    setSaving(true)
    const u = JSON.parse(localStorage.getItem('user') || '{}')
    const adjustments: Array<{ productId: string; countedQuantity: number }> = []
    ...
    if (adjustments.length === 0) {
      setMsg('Aucun ecart a appliquer')
      setSaving(false)
      return
    }
    ...
    if (!response.ok) {
      alert(result?.error || 'Inventaire impossible')
      setSaving(false)
      return
    }

    setMsg(`OK ${result?.applied || 0} ajustement(s) applique(s)`)
    setCounts({})
    load()
    setSaving(false)
  }
    ...
    <div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18, flexWrap:'wrap', gap:10 }}>
        <div>
          ...
          <div style={{ fontSize:12, color:'#a8a69e', marginTop:2 }}>Saisissez les quantites reelles comptees</div>
        </div>
        ...
        <table style={{ width:'100%', borderCollapse:'collapse', minWidth:700 }}>
          <thead>
            <tr style={{ background:'#f0eeea' }}>
              {['Produit', 'Stock theorique', 'Stock compte', 'Ecart'].map((h) => (
      ...
      <button onClick={applyInventory} disabled={saving || Object.keys(counts).length === 0}
        ...
        {saving ? 'Application...' : `Appliquer les ecarts (${Object.keys(counts).filter((k) => counts[k]).length} ligne(s))`}
      </button>
    </div>
  )
}
