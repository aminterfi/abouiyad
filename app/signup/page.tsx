...
function friendlySignupError(message = '') {
  if (isAlreadyRegisteredError(message)) {
    return 'Cet email est deja utilise. Veuillez vous connecter.'
  }
  if (message.includes('Invalid email')) return 'Adresse email invalide.'
  if (isDuplicateError(message)) {
    return 'Cette entreprise, ce lien ou cet email existe deja.'
  }
  if (message.includes('violates row-level security')) {
    return 'La creation est bloquee par les regles de securite Supabase.'
  }
...
export default function SignupPage() {
  ...
  async function hasCompanyForOwner(ownerId: string, slug: string, companyName: string) {
    const { data, error } = await supabase.rpc('get_owner_companies', { p_user_id: ownerId })
    if (error || !Array.isArray(data)) return false

    return data.some((company: Record<string, unknown>) => {
      const candidateSlug = String(company.slug || company.company_slug || '').toLowerCase()
      const candidateName = String(company.company_name || '').toLowerCase()
      return candidateSlug === slug.toLowerCase() || candidateName === companyName.toLowerCase()
    })
  }
  ...
  async function createCompanyForOwner(ownerId: string, email: string, companyName: string, slug: string, fullName: string, phone: string | null) {
    ...
    for (const attempt of attempts) {
      ...
      if (isDuplicateError(error.message)) {
        const alreadyLinked = await hasCompanyForOwner(ownerId, slug, companyName)
    ...
    throw new Error(finalError ? `Impossible de creer l'entreprise: ${finalError.message}` : 'Impossible de creer l\'entreprise.')
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (!form.password || form.password.length < 6) { setError('Le mot de passe doit contenir au moins 6 caracteres.'); return }
    ...
    try {
      ...
      if (!authErr && authData.user?.identities?.length === 0) {
        ...
        if (loginErr || !loginData.user) {
          throw new Error('Cet email existe deja. Connectez-vous avec le bon mot de passe, ou utilisez un autre email.')
        }
        ...
        if (!authData.user) throw new Error('Impossible de creer le compte utilisateur.')
    ...
    return (
      <div style={pageStyle}>
        <div style={{ ...cardStyle, maxWidth: 440, textAlign: 'center' }}>
          <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#dcfce7', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', fontSize: 30, marginBottom: 16 }}>OK</div>
          <div style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Compte cree avec succes !</div>
          <div style={{ fontSize: 13, color: '#6b6860', marginBottom: 20, lineHeight: 1.5 }}>
            Votre periode d&apos;essai de <strong>14 jours</strong> a commence.<br />
            Confirmation envoyee a <strong>{form.email}</strong>
          </div>
    ...
    <div style={pageStyle}>
      <div style={{ width: '100%', maxWidth: 480 }}>
        ...
        <div style={{ textAlign: 'center', marginBottom: 24 }}>
          ...
          <div style={{ fontSize: 26, fontWeight: 800, color: '#1a1916', letterSpacing: '-.5px' }}>Creer votre entreprise</div>
        ...
        <div style={cardStyle}>
          ...
          <form onSubmit={submit}>
            <div style={{ marginBottom: 14 }}>
              ...
              <input required autoFocus value={form.company_name} onChange={e => updateCompanyName(e.target.value)} placeholder="Ma Societe SARL" style={inputStyle} />
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 14 }}>
              <div>
                ...
                <input required value={form.full_name} onChange={e => setForm({ ...form, full_name: e.target.value })} placeholder="Prenom Nom" style={inputStyle} />
              </div>
              <div>
                <label style={{ fontSize: 12, fontWeight: 500, color: '#6b6860', marginBottom: 6, display: 'block' }}>Telephone</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginBottom: 20 }}>
              <div>
                ...
                <input type="password" required value={form.password} onChange={e => setForm({ ...form, password: e.target.value })} placeholder="Min. 6 caracteres" style={inputStyle} />
              </div>
              <div>
            ...
            <div style={{ padding: '12px 14px', background: 'rgba(91,61,245,0.05)', border: '1px solid rgba(91,61,245,0.15)', borderRadius: 8, fontSize: 11, color: '#5B3DF5', marginBottom: 16 }}>
              <strong>Periode d&apos;essai 14 jours :</strong> Acces complet a toutes les fonctionnalites.
            </div>
            ...
            <button type="submit" disabled={loading} style={{ width: '100%', padding: 14, fontSize: 14, fontWeight: 600, background: loading ? '#a8a69e' : 'linear-gradient(135deg, #2563EB, #5B3DF5)', color: '#fff', border: 'none', borderRadius: 8, cursor: loading ? 'not-allowed' : 'pointer', fontFamily: 'inherit' }}>
              {loading ? 'Creation en cours...' : 'Creer mon entreprise'}
            </button>

            <div style={{ textAlign: 'center', marginTop: 16, fontSize: 12, color: '#6b6860' }}>
              Deja un compte ? <Link href="/" style={{ color: '#2563EB', fontWeight: 600, textDecoration: 'none' }}>Se connecter</Link>
            </div>
          </form>
        </div>
      </div>
